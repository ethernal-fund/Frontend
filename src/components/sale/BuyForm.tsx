...
export function BuyForm({
  ...
  const setMax = () => setValue('usdcAmount', Math.min(Number(usdcBalance), maxWallet).toFixed(2))
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      setTxError(e?.shortMessage ?? e?.message ?? t('buy.error.rejected'))
      setStep('input')
    }
  }
    ...
    return (
      <div className="flex flex-col items-center py-10 text-center">
        <div
          ...
          style={{ background: 'rgba(137,113,72,0.15)', border: '0.5px solid rgba(196,169,106,0.4)' }}
        >
          <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
            <path d="M4 11L9 16L18 6" stroke="#c4a96a" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </div>
        <h3 className="text-xl font-light mb-2" style={{ color: '#f7f8f6', fontFamily: "'Cormorant Garamond', Georgia, serif" }}>
          {t('buy.success.title')}
        </h3>
        <p className="text-sm mb-1" style={{ color: '#666' }}>
          {t('buy.success.bought', { amount: Number(tokensOut).toLocaleString('en-US', { maximumFractionDigits: 2 }) })}
        </p>
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <div>
        <div className="flex justify-between items-baseline mb-2">
          ...
          <button
            ...
            style={{ color: '#666' }}
          >
            {t('buy.max', { amount: Number(usdcBalance).toLocaleString('en-US', { maximumFractionDigits: 2 }) })}
          </button>
        </div>
      ...
      >
        <div className="flex justify-between items-center">
          <span className="text-xs tracking-widest uppercase" style={{ color: '#666' }}>{t('buy.youWillReceive')}</span>
          <span className="text-lg font-light" style={{ color: '#c4a96a', fontFamily: "'Cormorant Garamond', Georgia, serif" }}>
            {Number(tokensOut) > 0
        ...
        <div className="flex justify-between items-center">
          <span className="text-xs tracking-widest uppercase" style={{ color: '#555' }}>{t('buy.pricePerToken')}</span>
        ...
        >
          <div className="flex justify-between items-center">
            <span className="text-xs tracking-widest uppercase" style={{ color: '#555' }}>{t('buy.cliff')}</span>
            <span className="text-xs" style={{ color: '#666' }}>
              {t('buy.cliffMonths', { months: round.cliffMonths })}
            </span>
          </div>
          <div className="flex justify-between items-center mt-1.5">
            <span className="text-xs tracking-widest uppercase" style={{ color: '#555' }}>{t('buy.vesting')}</span>
            <span className="text-xs" style={{ color: '#666' }}>
      ...
      {txError && (
        <div
          ...
          style={{ background: 'rgba(224,82,82,0.08)', border: '0.5px solid rgba(224,82,82,0.25)', color: '#e05252' }}
        >
          {txError}
        </div>
      )}
      ...
      {(step === 'approving' || step === 'buying') && (
        <div className="flex items-center gap-3">
          <StepDot active={step === 'approving'} done={step === 'buying'} label={t('buy.step1')} />
          <div className="flex-1 h-px" style={{ background: 'rgba(255,255,255,0.08)' }} />
          <StepDot active={step === 'buying'} done={false} label={t('buy.step2')} />
        </div>
      )}
  ...
  return (
    <div className="flex items-center gap-1.5">
      <div
        ...
        style={{
          background: done   ? '#897148'
                    : active ? 'rgba(137,113,72,0.2)'
      ...
      >
        {done && (
          <svg width="8" height="8" viewBox="0 0 8 8" fill="none">
            <path d="M1.5 4L3.5 6L6.5 2" stroke="#f7f8f6" strokeWidth="1.2" strokeLinecap="round"/>
          </svg>
        )}
        {active && !done && (
          <div className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: '#c4a96a' }} />
        )}
      </div>
      <span className="text-[10px] tracking-widest uppercase" style={{ color: active || done ? '#c4a96a' : '#444' }}>
        {label}
      </span>
    </div>
  )
...
function BuyButton({
  step, needsApproval, isPending, disabled, t,
}: { step: Step; needsApproval: boolean; isPending: boolean; disabled: boolean; t: (key: string) => string }) {
