...
import { Shield, GraduationCap, TrendingUp, ArrowRight, FileText, Download, ScrollText } from 'lucide-react';
...
interface LegalDoc {
  ...
  files: { lang: string; href: string }[];
}

const LEGAL_DOCS: LegalDoc[] = [
  {
    labelKey: 'legal.whitepaper',
    icon: ScrollText,
    files: [
      { lang: 'ES', href: '/legal/Ethernal_Fund_Whitepaper_v1_0_ES.pdf' },
      { lang: 'EN', href: '/legal/Ethernal_Fund_Whitepaper_v1_0_EN.pdf' },
    ],
  },
  {
    labelKey: 'legal.tokenSaleAgreement',
    icon: FileText,
    files: [
      { lang: 'ES', href: '/legal/Etrf_Token_Sale_Agreement_v1_0_ES.pdf' },
      { lang: 'EN', href: '/legal/Etrf_Token_Sale_Agreement_v1_0_EN.pdf' },
    ],
  },
];
      ...
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-10">
          <h2 className="text-2xl font-bold text-gray-800 mb-2">
            {t('legal.sectionTitle', 'Documentación Legal')}
          </h2>
          <p className="text-gray-500 text-sm max-w-xl mx-auto">
            {t(
              'legal.sectionSubtitle',
              'Todos los documentos del protocolo están disponibles públicamente. Leelos antes de participar.',
            )}
          </p>
        </div>

        <div className="grid sm:grid-cols-2 gap-6">
          {LEGAL_DOCS.map(({ labelKey, icon: Icon, files }) => (
            <div
              key={labelKey}
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">v1.0 · Mayo 2026</p>
                </div>
              </div>
              ...
              <div className="flex gap-3 mt-auto pt-2 border-t border-gray-100">
                {files.map(({ lang, href }) => (
                  <a
                    ...
                    download
                    className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-lg border border-gray-300 text-gray-700 hover:border-green-500 hover:text-green-700 hover:bg-green-50 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-green-400"
                    aria-label={`${t(labelKey, labelKey)} — ${lang} (PDF)`}
                  >
                    <Download size={14} />
                    {lang}
